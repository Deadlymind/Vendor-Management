...
def update_vendor_metrics(sender, instance, created, **kwargs):
    ...
    # Update metrics only if specific fields have changed or it's a new instance
    fields_to_check = ['status', 'quality_rating', 'acknowledgment_date', 'delivery_date']
    if created or any(instance.tracker.has_changed(field) for field in fields_to_check):
...
def update_on_time_delivery_rate(vendor):
    try:
        ...
        on_time = PurchaseOrder.objects.filter(vendor=vendor, status='Completed', delivery_date__lte=F('order_date')).count()
        if total_completed > 0:
            vendor.on_time_delivery_rate = (on_time / total_completed) * 100
    except Exception as e:
        logger.error(f"Error updating on-time delivery rate for vendor {vendor.id}: {str(e)}")
...
def update_quality_rating_average(vendor):
    try:
        ...
        if ratings.exists():
            vendor.quality_rating_avg = ratings.aggregate(Avg('quality_rating'))['quality_rating__avg']
    except Exception as e:
        logger.error(f"Error updating quality rating average for vendor {vendor.id}: {str(e)}")
...
def update_average_response_time(vendor):
    try:
        ...
        if response_times.exists():
            total_response_time = sum((rt.acknowledgment_date - rt.issue_date).total_seconds() for rt in response_times)
            vendor.average_response_time = (total_response_time / len(response_times)) / 3600  # Convert seconds to hours
    except Exception as e:
        logger.error(f"Error updating average response time for vendor {vendor.id}: {str(e)}")
...
def update_fulfillment_rate(vendor):
    try:
        ...
        logger.error(f"Error updating fulfillment rate for vendor {vendor.id}: {str(e)}")
